...
# Relative to the current working directory -- see cli.py's note on _DEFAULT_MODEL_PATH for why
# (a __file__-based default breaks under a regular, non-editable install).
DEFAULT_PATH = Path("ignore-list.txt")

HEADER = (
    "# orientation-correct ignore list -- one absolute path per line.\n"
    "# Files here are always skipped, even if not (or no longer) backed up.\n"
    "# Populated by `python -m orientation_correction.revert`; edit by hand if you like.\n"
)
