...
### PV1 — Date heuristic false positives (undated misclassification) — ✅ FIXED (twice)
...
- **Fix v2**: switched to comparing `date_original` (osxphotos's EXIF-at-import value, which
  mirrors `date_added` *exactly* only when there was no real EXIF date) against `date_added`,
  with a much tighter threshold (2s vs. 60s). Validated against 681 real, *locally-available*
  assets on the original spike library: 34 → 6 assets flagged undated, two independent ground
  truths agreeing, looked complete.
- **It wasn't complete.** Before touching the user's real target library, wrote
  `scripts/verify_date_heuristic_fix.sh` — a script that installs the actual fixed package from
  the public repo (not a reimplementation) and runs the same before/after comparison against any
  real library. Ran it against the *same* spike library but checking all 10,267 assets instead of
  just the locally-available 681, and it caught v2 still misclassifying 64 assets —
  disproportionately videos, exclusively cloud-only, with real gaps as small as 0.348s (cloud-only
  synced metadata apparently processes faster than a full local import, producing tighter gaps
  than the locally-available validation set ever showed).
- **Fix v3**: threshold dropped to 100ms (>3x margin below the 0.348s floor, found empirically
  across the *complete* library this time). Re-verified: 35 → 6 flagged undated, zero unexpected
  new flags. Full technical writeup in design.md
  [Section 5.2](design.md#52-date-resolution--the-undated-heuristic).
- Touched: `date_resolver.py` (new `date_original` parameter, threshold revised twice: 2s → 100ms),
  ...
  tests for both false-positive patterns found, plus the screenshot edge case that turned out to
  never actually be at risk either way).
